...
if(Launcher.isBlockLauncher() || Launcher.isToolbox())function screenChangeHook(screenName) {
...
function mainMenu2(){
	...
	ctx.runOnUiThread(new Runnable({ run: function(){
		...
		try{
			...
			if(GetLang == "fr_FR")uidebug.setText('-Interface de Développeur');
...
function mainMenu4(){
	...
	ctx.runOnUiThread(new Runnable({ run: function(){
		...
		try{
			...
			changelog.setText("Adds : Spawn Fireball and Unlimited Items");

			if(GetLang == "fr_FR")changelog.setText('Ajoutes : Apparaître une Boule de Feu et Unlimited Items');
			...
			changelog1.setText("Fix : All Bugs");

			if(GetLang == "fr_FR")changelog1.setText('Corrige : Tout les Bugs');
...
function deathHook(a, v) {
	...
	if(morphkill) {
		...
		Entity.setRenderType(getPlayerEnt(), v);
